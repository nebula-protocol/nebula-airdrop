...
void (async function () {
  ...
  // take snapshot
  const airdropSnapshot = await takeSnapshot(snapshotService, +height);
...
async function takeSnapshot(
  ...
  fs.writeFileSync(
    "./airdrop_raw.json",
    JSON.stringify(delegatorsNebula, null, 4),
    "utf-8"
  );

  // remove delegators whose allocation is smaller than 1 uNEB
  let cleanedDelegatorsNebula: Record<string, number> = Object.keys(
    delegatorsNebula
  ).reduce((acc, each) => {
  ...
  let i = 0;
  const airdropSnapshot = Object.entries(cleanedDelegatorsNebula).map(
    (account) => {
      ...
      if (i % 100 === 0) {
        logger.info(
          `-- Merkle item example: ${merkleItem.delegator} getting ${merkleItem.amount} uNEB with ${merkleItem.proof}`
        );
      }
      i++;
      return merkleItem;
